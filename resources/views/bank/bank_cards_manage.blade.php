      ...
      <div class="am-g">
        ...
        <div class="am-u-sm-12 am-u-md-3">
          <div class="am-form-group">
            <select data-am-selected="{btnSize: 'sm'}" name="cardType">
              <option value=" " {{$cardType==" "?"selected":""}}>全部</option>
              <option value="menu" {{$cardType=="menu"?"selected":""}}>菜单类</option>
              <option value="operation" {{$cardType=="operation"?"selected":""}}>操作类</option>
            </select>
          </div>
        </div>
        <div class="am-u-sm-12 am-u-md-3">
              ...
              <tbody id="tbody">
                @foreach ($bankCards as $bankCard)
                 <tr>
                  ...
                  <td><input class='bankCard_id' type="hidden" value="{{ $bankCard->id }}" >{{ $bankCard->id }}</td>
                  <td>{{$bankCard->cardName}}</td>
                  <td>{{$bankCard->cardBank}}</td>
                  <td class="am-hide-sm-only">{{$bankCard->cardInfo}}</td>
                  ...
                  <td>
                    <div class="am-btn-group am-btn-group-xs">
                      ...
                      <button type="button" class="am-btn {{$bankCard->bankCard_status==1?'am-btn-warning':'am-btn-success'}}  am-radius single_enabled"><span class="am-icon-copy"></span> {{$bankCard->bankCard_status==1?'禁用':'启用'}}</button>
